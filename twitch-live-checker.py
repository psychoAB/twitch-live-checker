...
DEFAULT_CONFIG_FILE_PATH = \
    pathlib.Path.home() / pathlib.Path('.twitch-live-checker.conf')
...
MAX_STREAMER_STATUS_ENUM_LENGTH = \
    max(map(
        len,
        [streamer_status_enum.value
            for streamer_status_enum in StreamerStatus]))
...
def main():
    ...
    streamer_status_dict = \
        {
            streamer:
                [StreamerStatus.waiting,
                    TAG_NOT_FOUND_STRING]
    ...
    thread_render = threading.Thread(
        target=print_main_output_loop,
        args=(streamer_status_dict, username_not_valid_list),
        daemon=True)
    thread_render.start()
    ...
    while (
            ((streamer_queue.empty() is False) or
                (threading.active_count() > 2) or
                (len(time_streamer_request_prev_dict) > 0)) and
            (flag_should_stop.is_set() is False)):
        ...
        streamer_retrying_ready_list = \
            list(filter(
                lambda streamer: (
                    (time.time() -
                        time_streamer_request_prev_dict[streamer]) >
                    RETRY_INTERVAL),
                time_streamer_request_prev_dict))
        ...
        streamer_not_found_list = \
            list(filter(
                lambda streamer: (
                    streamer_retry_count_dict[streamer] > RETRY_LIMIT),
                streamer_retrying_ready_list))

        streamer_retrying_ready_list = \
            list(filter(
                lambda streamer: (
                    streamer not in streamer_not_found_list),
                streamer_retrying_ready_list))
        ...
        if (
                (streamer_queue.empty() is False) and
                (threading.active_count() < thread_num_max + 2) and
                (request_count < REQUEST_PER_SECOND_LIMIT)):
            streamer = streamer_queue.get()

            threading.Thread(
                target=check_streamer_status,
                args=(
                    streamer,
                    streamer_status_dict,
                    time_streamer_request_prev_dict),
    ...
    if exception_thread is not None:
        if (
                (type(exception_thread) is urllib.error.URLError) and
                (type(exception_thread.reason) is socket.gaierror)):
...
def check_streamer_status(
    ...
    if streamer_html_content.find('isLiveBroadcast') != -1:
        ...
        streamer_tag_position_start = \
            streamer_html_content.find('/directory/category/')

        if streamer_tag_position_start != -1:
            streamer_tag_position_start = (
                streamer_html_content.find(
                    '>',
                    streamer_tag_position_start + 1)
                + 1)
            streamer_tag_position_end = \
                streamer_html_content.find(
                    '<',
                    streamer_tag_position_start + 1)
            ...
            streamer_tag = \
                streamer_html_content[
                    streamer_tag_position_start: streamer_tag_position_end]
...
def get_streamer_html_content(streamer):
    ...
    try:
        ...
        if type(exception.reason) is TimeoutError:
            streamer_html_content = ''
        else:
            raise exception
...
def parse_config(config_file_text):
    ...
    config_file_text_line = \
        list(filter(lambda line: line != '', config_file_text_line))
    ...
    if len(config_file_text_line) > 0:
        if (
                re.fullmatch(
                    '[1-9][0-9]*',
                    config_file_text_line[0]) is not
                None):
            thread_num_max = int(config_file_text_line[0])

            config_file_text_line = config_file_text_line[1:]

    username_not_valid_list = \
        list(filter(
            lambda line: (
                re.fullmatch(r'[a-zA-Z0-9]\w{3,24}', line) is None),
            config_file_text_line))
    streamer_list = \
        list(filter(
            lambda line: line not in username_not_valid_list,
            config_file_text_line))
...
def print_main_output_loop(streamer_status_dict, username_not_valid_list):
    ...
    while flag_should_stop.is_set() is False:
...
def print_main_output(streamer_status_dict, username_not_valid_list):
    ...
    if len(streamer_status_dict) > 0:
        ...
        streamer_string_length_max = \
            max(map(len, streamer_status_dict.keys()))

        for streamer in streamer_status_dict.keys():
            print_streamer_status(
                streamer,
                streamer_status_dict[streamer][0].value,
                streamer_status_dict[streamer][1],
                streamer_string_length_max)

        lock_streamer_status_dict.release()
    else:
        print_to_stderr(
            'No streamer to check, check your configuration file.')
...
def print_streamer_status(
    ...
    print('{}\t'.format(
        streamer.ljust(streamer_string_length_max)), end='')
    print('{}\t'.format(
